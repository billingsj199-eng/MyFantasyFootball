...
def main():
    ...
    for wk in range(1, last_week + 1):
        ...
        for sid, s in stats.items():
            ...
            if pos not in FPA_POS or team not in finals:
                continue
            ...
            slot[pos] = round(slot.get(pos, 0) + pts, 2)
        for sid, s in stats.items():
            hit = by_sid.get(sid)
            if not hit:
                continue
...
def write_fpa(fpa_weeks):
    ...
    out = ('// AUTO-GENERATED by scripts/pull_postgame_stats.py - fantasy points ALLOWED per defense per week\n'
           '// ALL Sleeper QB/RB/WR/TE in FINAL games (half-PPR), not just board players. opp = offense faced.\n'
