...
(() => {
  ...
  // v0.29.15: jacks-official is premium-only read since 2026-08-13, so the
  ...
  async function refreshJackBoards() {
    ...
    // PREMIUM (v0.28.2): the full-board pull is premium-only. Read the
    ...
    if (premium) {
      try { doc = await bgFetch(FIRESTORE_URL); full = true; } catch (e) {}
    }
    if (!doc) {
      try { doc = await bgFetch(FIRESTORE_PUBLIC_URL); } catch (e) { return; }
      full = false;
    }
    try {
  ...
  // on bye / OUT, discounted for Doubtful/Questionable, nudged by Jack's
  // redraft rank (±1.5 ppg); K/DST fallback = the league-rule Vegas models above.
  ...
  // WEEK PIN (0.29.35, Jack 09-15: "showing week 1 scores for week 2"). Without
  ...
  function scoreboardQuery() {
    const season = (state.nflState && (state.nflState.season || state.nflState.league_season)) || '';
  ...
  async function refreshScoreboard(force) {
    ...
    try {
      const sb = await bgFetch(SCOREBOARD_URL + scoreboardQuery() + '&t=' + now);
  ...
  function liveTeamOf(p) { return (p && (p.sTm || p.t)) || null; }
  ...
  // league, so he is in livePts): the starter is out.
  function liveTeammateQB(k, tm) {
    if (!tm) return false;
    for (const o of Object.keys(state.livePts)) {
      if (o === k || !(state.livePts[o] >= 2)) continue;
      const q = state.byId[o] || state.slMeta[o];
      if (q && (q.s || q.pos) === 'QB' && (q.sTm || q.tm) === tm) return true;
    }
    return false;
  }
  ...
  function wkLive(p) {
    ...
    if (!g) { livePreSeed(p, sid); return { live: proj, actual: null, rem: proj, f: 0, st: 'pre', tag: '', proj }; }
