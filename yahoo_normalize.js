/* MFF Yahoo League Import — yahoo_normalize.js
 *
 * SYNCED COPY of yahoo-extension/yahoo-normalize.js (the extension folder is
 * untracked, so the site needs its own deployable copy). If you edit the
 * extension's yahoo-normalize.js, re-copy it here verbatim below this header.
 * Lazy-loaded by app.js (_mtLoadYahooNormalizer) for the direct league sync
 * (league HTML arrives via the yahooProxy Cloud Function; Yahoo sends no
 * CORS headers for our origin).
 */
